...
Part = namedtuple("Part", ['id', 'input_file', 'solver_file', 'engine', 'time_limit', 'name'])


def load_metadata(metadata_file_name='C://Users//HP//Desktop//facility//_coursera'):
    '''
...
def compute(metadata, solver_file_override=None):
    ...
    for problem in selected_problems:
        ...
        if not os.path.isfile("C://Users//HP//Desktop//facility//solverCpp.py"):
        ...
        submission = output(problem.input_file, solver_file, problem.engine, problem.time_limit)
        if submission != None:
...
def output(input_file, solver_file, engine, time_limit):
    '''
    ...
    start = time.clock()
    try:
        solution = pkg.solve_it(load_input_data(input_file), engine, time_limit)
    except Exception as e:
